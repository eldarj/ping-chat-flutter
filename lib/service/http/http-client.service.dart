...
const String API_BASE_URL = 'http://46.101.169.71:8089';
...
class HttpClientService {
  ...
  static Future<http.Response> getQuery(endpoint, queryMap) async {
    ...
    var response = await http.get(
        Uri.http('46.101.169.71:8089', endpoint, queryMap),
        headers: {'content-type': 'application/json', 'authorization': 'Bearer $userToken'}
    ).timeout(Duration(seconds: 10));
  ...
  static void postStream(String url, String filename, File file) async {
    ...
    final streamedRequest = new http.StreamedRequest('POST', Uri.http('46.101.169.71:8089', url))
      ..headers.addAll({
